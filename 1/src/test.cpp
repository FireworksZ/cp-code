...
// 分治合并函数：合并区间 [l, r] 的分式
Fraction solve_dac(int l, int r, const vector<Fraction>& leaves) {
    if (l == r) return leaves[l];
    int mid = (l + r) >> 1;
    Fraction resL = solve_dac(l, mid, leaves);
    Fraction resR = solve_dac(mid + 1, r, leaves);

    // 合并逻辑: num = numL * denR + numR * denL
    //          den = denL * denR
    Poly::poly term1 = Poly::poly_mul(resL.num, resR.den);
    Poly::poly term2 = Poly::poly_mul(resR.num, resL.den);

    Poly::poly new_num(max(term1.size(), term2.size()));
    for (size_t i = 0; i < new_num.size(); ++i) {
        int v1 = (i < term1.size()) ? term1[i] : 0;
        int v2 = (i < term2.size()) ? term2[i] : 0;
        new_num[i] = Poly::plus(v1, v2);
    }

    Poly::poly new_den = Poly::poly_mul(resL.den, resR.den);
    return {new_num, new_den};
}

void solve() {
    // -----------------------------------------------------
    // 第一部分: Power Projection (你已经写好的部分)
    // -----------------------------------------------------
    // 注意: init 的大小根据题目N决定，N=10^5~10^6 级别通常 21 够用
    // 如果 N 很大 (如 5e6)，可能需要 init(23)
    Poly::init(21);
    ...
    for (int i = 0; i < m; i++) {
        int x;
        cin >> x;
        if (x < n) d[x] = Poly::plus(d[x], inv_m); // 累加概率，防止重复面
    }
    ...
    // 计算 f[0]...f[n-1]
    // f[i] 表示走了 i 步还没有到达终点的概率
    auto f = Poly::power_projection(d, c);

    // f 需要扩充一项 f[n]=0 (因为超过 N 步肯定结束了或 f_n 定义为0)
    // 方便后面统一处理 k=1...n
    f.resize(n + 1, 0);
//    for(auto e:f) cout<<e<<" ";
    ...
    // 1. 预处理需要合并的分式叶子节点
    vector<Fraction> leaves;

    // 单独计算第 L 个人的答案 (i=L)
    // 公式: sum_{k=1}^N g_k * f_k^{L-1}
    int ans_L = 0;

    for (int k = 1; k <= n; ++k) {
        // g_k = f_{k-1} - f_k
        // 注意 f[k] 可能因为 resize 默认为 0
        int fk = (k < (int)f.size()) ? f[k] : 0;
        int fk_prev = f[k - 1];
        int gk = Poly::minus(fk_prev, fk);

        // 计算 Person L 的贡献
        int term_L = 1ll * gk * qpow(fk, l - 1) % mod;
        ans_L = Poly::plus(ans_L, term_L);

        // 准备 Person 1...(L-1) 的分式
        // 如果 f_{k-1} == 0，则该项为 0，跳过
        if (fk_prev == 0) continue;

        // 构造分式:
        // 分子 Num = g_k * f_{k-1}^{L-1}
        // 分母 Den = 1 - (f_k / f_{k-1}) * x

        int num_const = 1ll * gk * qpow(fk_prev, l - 1) % mod;
        int ratio = 1ll * fk * qpow(fk_prev, mod - 2) % mod; // f_k / f_{k-1}

        Poly::poly leaf_num = {num_const};
        Poly::poly leaf_den = {1, Poly::minus(0, ratio)}; // 1 - ratio * x

        leaves.push_back({leaf_num, leaf_den});
    }

    // 2. 分治合并
    // 得到总的大分式 P(x) / Q(x)
    Poly::poly P_final, Q_final;
    if (leaves.empty()) {
        P_final = {0};
        Q_final = {1};
    } else {
        Fraction res = solve_dac(0, leaves.size() - 1, leaves);
        P_final = res.num;
        Q_final = res.den;
    }

//    for(auto e: P_final) cout<<e<<" ";
//    cout<<endl;
//    for(auto e: Q_final) cout<<e<<" ";
//    cout<<endl;

    // 3. 计算 P(x) * Q(x)^{-1} mod x^{L-1}
    // 我们只需要前 L-1 项 (对应 i=1 到 L-1，指数为 0 到 L-2)
    // 但因为人是从 1 开始编号，[x^{i-1}] 对应第 i 个人
    // 所以我们需要计算到 x^{L-2} 即可，长度设为 L 比较安全

    int len = l+3;
    // 截断到需要的长度，减少求逆时间
    P_final.resize(len,0);
    Q_final.resize(len,0);

    Poly::poly Q_inv = Poly::poly_inv(Q_final, len);
    Poly::poly ResultPoly = Poly::poly_mul(P_final, Q_inv);

    for(int i=0;i<l;i++) cout<<ResultPoly[i]<<'\n';

//    ll sum = 0;
//    for(int i=0;i<l;i++) sum+=ResultPoly[i];
//    sum%=mod;
//    cout<<sum<<endl;
