...
namespace FPS
{
    ...
    void init(const int t) {//预处理出来NTT里需要的w和wn，砍掉了一个log的时间
        init_fac(1 << t); // 自动预处理阶乘，防止 fps_comp/fps_shift 未初始化
        for(int p = 1; p <= t; ++ p) {
    ...
    void NTT(fps &A, int type, int limit) {//快速数论变换
        A.resize(limit);
        for(int i = 0; i < limit; ++ i)
            if(i < RR[i])
                swap(A[i], A[RR[i]]);
    ...
    fps fps_mul(const fps &A, const fps &B) {
        // 内部逻辑不变，但因为是引用，不能直接 resize A 或 B
        // 你需要创建新的临时变量
        if (A.empty() || B.empty()) return {};
        int len = A.size() + B.size() - 1;
        int limit = NTT_init(len);

        fps FA(A.begin(), A.end()); FA.resize(limit); // 显式拷贝并 resize
        fps FB(B.begin(), B.end()); FB.resize(limit);
    ...
    fps fps_inv(fps &f, int len) {//多项式求逆
    		if (f.empty()) return {}; // ✅ 防止空向量访问 f[0]
    	    if (len == 1)
    	        return fps(1, qpow(f[0], mod - 2));

        int input_len = min((int)f.size(), len);
        fps A(f.begin(), f.begin() + input_len);
        A.resize(len, 0);
        fps B = fps_inv(f, (len + 1) >> 1);
    ...
    fps fps_ln(fps f, int len) {//多项式求对数
        fps A = fps_idev(fps_mul(fps_dev(f), fps_inv(f, len)));
        return A.resize(len), A;
    }

    fps fps_exp(fps &f, int len) {//多项式求指数
        if(len == 1)
            return fps(1, 1);

        fps B = fps_exp(f, (len + 1) >> 1);
        B.resize(len);
        fps lnB = fps_ln(B, len);
        for(int i = 0; i < len; ++ i) {
			int fi = (i < (int)f.size()) ? f[i] : 0; // 安全获取 f[i]
			lnB[i] = ck(fi - lnB[i] + mod);
		}
        int limit = NTT_init(len << 1);//n -> n^2
    ...
    fps fps_sqrt(fps &f, int len) {//多项式开方
    		assert(f.size()&&f[0]==1);
        if(len == 1) return fps(1, 1);
        int input_len = min((int)f.size(), len);
		fps A(f.begin(), f.begin() + input_len);
		A.resize(len, 0);
        fps B = fps_sqrt(f, (len + 1) >> 1);
        fps IB = fps_inv(B, len);
        int limit = NTT_init(len << 1);
    ...
    fps fps_pow(fps f, int k) {//多项式快速幂
    		assert(f.size()&&f[0]==1);
        f = fps_ln(f, f.size());
        for(auto &x : f) x = 1ll * x * k % mod;
        return fps_exp(f, f.size());
    }

    fps fps_rev(const fps &f, int len) {//多项式翻转
		fps g(len);
		int n = f.size();
		for(int i = 0; i < len; ++ i) {
			if (n - 1 - i >= 0 && n - 1 - i < n)
    ...
    //多项式相除求商和余数
	pair<fps, fps> fps_div(fps F, fps G) {
		// F(x) = Q(x)G(x) + R(x)
		int n = F.size(), m = G.size();
		if (n < m) { // 被除式次数小于除式次数
			return {fps(1, 0), F}; // 商为 0，余为 F
		}
		...
		fps FR = fps_rev(F, n); // F^R, deg: n-1
		fps GR = fps_rev(G, m); // G^R, deg: m-1
		GR.resize(len_Q);

		// 2. 求 (G^R)^-1 mod x^(k+1)
		fps inv_GR = fps_inv(GR, len_Q);
		...
		FR.resize(len_Q);
		fps QR = fps_mul(FR, inv_GR);
		QR.resize(len_Q);

		// 4. 翻转得到商式 Q(x)
		// Q(x) 次数为 k，所以 Q^R(x) 的系数应取前 k+1 项 (0到k次)
		fps Q = fps_rev(QR, len_Q); // Q(x) 长度为 k+1, deg: k
    ...
    fps fps_cos(fps f, int len) {//多项式三角函数（cos）
    		int input_len = min((int)f.size(), len);
    	    fps A(f.begin(), f.begin() + input_len);
    	    A.resize(len, 0);
        fps B(len), C(len);
    ...
    fps fps_sin(fps f, int len) {//多项式三角函数（sin）
    		int input_len = min((int)f.size(), len);
    	    fps A(f.begin(), f.begin() + input_len);
    	    A.resize(len, 0);
        fps B(len), C(len);
    ...
    fps fps_arcsin(fps f, int len) {
        fps A(f.size()), B(f.size()), C(f.size());
        A = fps_dev(f);
        B = fps_mul(f, f);
    ...
    fps fps_arctan(fps f, int len) {
        fps A(f.size()), B(f.size()), C(f.size());
        A = fps_dev(f);
        B = fps_mul(f, f);
        if ((int)B.size() < len) B.resize(len, 0);
        B[0] = plus(B[0], 1);
        C = fps_inv(B, len);
        C = fps_mul(A, C);
        C = fps_idev(C);
	...
	void build_tree(const vector<int> &x, int i, int l, int r) {
		...
		M[i] = fps_mul(M[i << 1], M[i << 1 | 1]);
	}

	// 2. 多项式取模（直接调用 fps_div 并返回余数 R）
	fps fps_mod(fps F, fps G) {
		// F(x) mod G(x) = R(x)
		int n = F.size(), m = G.size();
		if (n < m) return F; // deg(F) < deg(G), 余数即为 F

		// 调用 fps_div，我们只需要余数 R
		// fps_div 返回 {Q, R}
		return fps_div(F, G).second;
	}

	// 3. 分治求值
	void solve_eval(fps A, int i, int l, int r, vector<int> &res) {
		if (l == r) {
	...
	// F: 待求值的多项式
	// x: 求值点集合
	vector<int> fps_eval(fps F, const vector<int> &x) {
		int m = x.size();
		if (m == 0) return {};
	...
	fps solve_interp(const vector<int> &x, const vector<int> &y,
		...
		fps P_L = solve_interp(x, y, inv_Mx, i << 1, l, mid);
		fps P_R = solve_interp(x, y, inv_Mx, i << 1 | 1, mid + 1, r);

		// 合并：P(x) = P_L(x) * M_R(x) + P_R(x) * M_L(x)
		// M_L(x) = M[i*2], M_R(x) = M[i*2+1]

		// P_L(x) * M_R(x)
		fps term1 = fps_mul(P_L, M[i << 1 | 1]);

		// P_R(x) * M_L(x)
		fps term2 = fps_mul(P_R, M[i << 1]);
	...
	fps fps_fast_interp(const vector<int> &x, const vector<int> &y) {
		int n = x.size();
		if (n == 0) return fps(1, 0);

		// 1. 构造模数树 M[i]
		// M 已经在 fps_eval 中定义并使用了，这里沿用 M
		M.resize(4 * n);
		...
		// 3. 计算 M'(x) (求导)
		fps Mx_dev = fps_dev(Mx);

		// 4. 多点求值计算 M'(x_i)
		// 注意：Mx_dev 的长度为 n (deg: n-1)，x 的长度为 n
		vector<int> Mx_dev_at_xi = fps_eval(Mx_dev, x);
		...
		// 6. 分治计算 P(x)
		fps P = solve_interp(x, y, inv_Mx, 1, 0, n - 1);
	...
	fps fps_extract(const fps &f, int type) {
		fps res;
		int n = f.size();
	...
	// -----------------------------------------------------------
	// 计算 [x^k] P(x) / Q(x)
	int bostan_mori(fps P, fps Q, long long k) {
		while (k > 0) {
			// 1. 预处理 Q 的奇偶部分
			fps Qe = fps_extract(Q, 0); // Q_e(x)
			fps Qo = fps_extract(Q, 1); // Q_o(x)

			// 2. 计算新分母 Q_new(x) = Q_e^2(x) - x Q_o^2(x)
			// 这一步与 k 的奇偶性无关，始终执行
			fps Qe_sq = fps_mul(Qe, Qe);
			fps Qo_sq = fps_mul(Qo, Qo);
			...
			// 3. 预处理 P 的奇偶部分
			fps Pe = fps_extract(P, 0);
			fps Po = fps_extract(P, 1);

			// 4. 计算新分子 P_new(x)，根据 k 的奇偶性选择不同的计算路径（优化掉一半的 FFT）
			if (k & 1) {
				// CASE: k 是奇数
				// 公式：P_new(x) = P_o(x)Q_e(x) - P_e(x)Q_o(x)
				fps Po_Qe = fps_mul(Po, Qe);
				fps Pe_Qo = fps_mul(Pe, Qo);
				...
				fps Pe_Qe = fps_mul(Pe, Qe);
				fps Po_Qo = fps_mul(Po, Qo);
	...
	fps fps_mul_len(const fps &A, const fps &B, int limit) {
		if (A.empty() || B.empty() || limit <= 0) return {};
		// 截断输入，只取有效部分
		fps FA(A.begin(), A.begin() + min((int)A.size(), limit));
		fps FB(B.begin(), B.begin() + min((int)B.size(), limit));

		fps res = fps_mul(FA, FB);
	...
	// k: 当前层级 (对应 shift = 2^k)
	// limit_deg: 目标模数次数
	fps solve_comp_opt(const fps &f, const vector<fps> &h_powers, int k, int limit_len) {
		...
		// 1. 分割 f 为左半部分和右半部分
		fps f_left(f.begin(), f.begin() + mid);
		fps f_right(f.begin() + mid, f.end());

		// 2. 递归求解
		fps res_left = solve_comp_opt(f_left, h_powers, k - 1, limit_len);

		// 关键优化：右半部分不仅要递归，还要乘上 G^mid
		// G^mid = x^mid * H^mid
		// 所以我们只需要计算 mod (limit_len - mid)
		...
		fps res_right = solve_comp_opt(f_right, h_powers, k - 1, limit_len - shift);

		// 3. 合并：res = res_left + (res_right * H^mid) << mid
		// 取出 H^mid (在 h_powers[k-1] 中)
		fps H_pow = h_powers[k - 1];

		// 计算乘积，注意长度限制减少了 shift
		fps term_right = fps_mul_len(res_right, H_pow, limit_len - shift);

		// 加法合并
		if (res_left.size() < limit_len) res_left.resize(limit_len);

		for (int i = 0; i < term_right.size(); ++i) {
			if (i + shift < limit_len) {
	...
	// 主函数
	fps fps_comp(fps f, fps g, int n) {
		f.resize(n);
		g.resize(n);
		...
		vector<fps> h_powers(log_m);
		if (log_m > 0) {
			h_powers[0] = h; // H^1
			if (h_powers[0].size() > n) h_powers[0].resize(n);

			for (int i = 1; i < log_m; ++i) {
				h_powers[i] = fps_mul_len(h_powers[i - 1], h_powers[i - 1], n);
			}
		}

		fps res = solve_comp_opt(f, h_powers, log_m, n);
		res.resize(n);
		return res;
	}
	...
	fps fps_inv_comp(fps F, int n) {
		...
		while (len < n) {
			...
			fps A = fps_comp(F_cut, G_cur, limit);
			...
			fps den_inv = fps_inv(A_dev, len); // 只做 len 长求逆，速度快一倍
			...
			// 直接乘比较安全，但我们可以限制结果长度。
			fps num = fps_mul(A, G_dev);
			// 我们只需要 num 的 [len, limit) 部分，但 fps_mul 算出了 [0, 2*len)
			// 这里的低位部分应该是 0
	...
	fps berlekamp_massey(const vector<int> &val) {
		fps C, oldC;
		C.push_back(1);
		oldC.push_back(1);
	...
	int kth_term_linear(const vector<int> &raw_seq, long long k) {
		...
		// Q = 1 - c_1 x - c_2 x^2 ...
		fps Q = berlekamp_massey(raw_seq);
		...
		fps A(raw_seq.begin(), raw_seq.begin() + min((int)raw_seq.size(), m));
		fps P = fps_mul(A, Q);
		if (P.size() > m) P.resize(m);
	...
	typedef vector<vector<int>> fps2d;

	// 辅助函数：矩阵转置
	// 将 R x C 的矩阵转置为 C x R
	void matrix_transpose(fps2d &Mat, int R, int C) {
		fps2d T(C, fps(R));
		for (int i = 0; i < R; ++i) {
			for (int j = 0; j < C; ++j) {
	...
	fps2d fps_mul_2d(const fps2d &A, const fps2d &B) {
	...
	// 假设 [x^0]f(x) = 0
	// 复杂度: O(N log^2 N)
	fps power_projection(fps f, fps g, int n = -1, int ny = -1) {
		if (n == -1) n = f.size() - 1;
		if (ny == -1) ny = n;
		...
		while (cur_n > 0) {
			...
			// S = P * R
			// T = Q * R = Q(x)Q(-x) = V(x^2)
			vector<fps> S = fps_mul_2d(P, R);
			vector<fps> T = fps_mul_2d(Q, R);
		...
		fps inv_Q = fps_inv(final_Q, target_deg);
		fps ans = fps_mul(final_P, inv_Q);
...
using FPS::fps;
using FPS::fps_arcsin;
using FPS::fps_arctan;
...
int main()
{
    FPS::init(21);//2^21 = 2,097,152,根据题目数据多项式项数的大小自由调整，注意大小需要跟deer数组同步(21+1=22)
    ...
    if(type == 0) g = fps_arcsin(f, n);
    else g = fps_arctan(f, n);
